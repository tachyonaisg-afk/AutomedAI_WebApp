...
const AdminResultEdit = () => {
    usePageTitle("Lab Test Result Edit");
    ...
    useEffect(() => {
        ...
        if (labTestData.normal_test_items) {
            const normalItems = labTestData.normal_test_items.map((item) => ({
                type: "normal",
                originalName: item.lab_test_name, // 🔥 IMPORTANT
            ...
            results.push(...normalItems);
        }

        // DESCRIPTIVE
        if (labTestData.descriptive_test_items) {
            const descItems = labTestData.descriptive_test_items.map((item) => ({
                type: "descriptive",
                originalName: item.lab_test_particulars,
                parameter: item.lab_test_particulars,
                value: item.result_value || "",
            }));

            results.push(...descItems);
        }

        // SENSITIVITY
        if (labTestData.sensitivity_test_items) {
            const sensItems = labTestData.sensitivity_test_items.map((item) => ({
                type: "sensitivity",
                originalName: item.antibiotic,
                parameter: item.antibiotic,
                value: item.result || "",
            }));

            results.push(...sensItems);
        }
        ...
        results = results.map((r) => {
            if (r.type === "normal") {
                return {
                    ...r,
                    flag: getFlagFromRange(
                        r.value,
                        r.normalRange,
                        labTestData?.patient_sex
                    ),
                };
            }
            return r;
            ...
            <ResultContainer>
                ...
                <ButtonGroup>
                    ...
                    <Button
                        ...
                        disabled={publishing || isSubmitted}
                    >
