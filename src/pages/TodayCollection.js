...
${pad(date.getHours())}:${pad(date.getMinutes())}:${pad(date.getSeconds())}`;
            ...
            const params = {
                ...
                // 🔴 TODAY FILTER
                filters: JSON.stringify([
                    ["creation", ">=", formatDateTime(start)],
                    ["creation", "<", formatDateTime(end)]
                ]),
            };
                ...
                const transformedData = response.data.data.map((item, index) => ({
                    ...
                    quantity_uom:
                        item.sample_qty && item.sample_uom
                            ...
                            : "-",
                    collection_datetime:
                        item.docstatus === 1 && item.collected_time
                            ? new Date(item.collected_time).toLocaleString("en-IN")
                            : "-",
                    collected_by:
                        item.docstatus === 1 && item.collected_by
                            ? item.collected_by.split("@")[0]
                            : "",
